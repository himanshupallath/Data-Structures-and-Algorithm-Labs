...
void swapWithNext(Node * p) {
    // Checks to see if it a head node or tail node or the node before the tail node
    if (p->value != 0 && (p->next)->value != 0) {
        Node * prev = p->prev;  // Keeps track of previous node
        Node * next = p->next;  // Keeps track of next node
        Node * nextNext = (p->next)->next;  // Keeps track of node after next node
        // Code below rearranges the pointers for swapping nodes
        prev->next = next;
        p->next = nextNext;
        next->next = p;
        next->prev = prev;
